...
Põhimõtteliselt võib *data frame*-i käsitleda kui ühepikkuste vektorite kogu, ühepikkused vektorid saamegi liita kokku *data frame*-ks.
...
```{r message = FALSE}
library(dplyr)
select(df, arvud) # anname ette eraldi argumentidena andmestiku ja tunnuse sellest andmestikust

mean(arvud) # anname ette eraldiseisva vektori
mean(df$arvud) # funktsioon töötab ka, kui defineerida ühes argumendis koos andmestik ja tunnus
```

Järgnevad käsud seevastu võimalikud poleks, annaksid veateate:

```{r eval = FALSE}
select(df$arvud) # antakse ette tunnus andmestikust ehk sisuliselt üks vektor - ei tööta
...
df <- data.frame(arvud, taimed, korgus)
```
...
```{r message = FALSE}
library(forcats)
df$korgus <- fct_relevel(df$korgus, levels = c("kõrge", "madal", "kõige madalam"))
freq(df$korgus)
```
...
<br>
Esitage Moodle's skriptifailis lahenduse kood koos kommentaaridega (mida mingi käsk teeb) ja sagedusjaotuse tabeli tõlgendustega.
