...
mean(df$arvud) # funktsioon töötab ka, kui anda ette tunnus andmestikust
```

Järgnevad käsud seevastu võimalikud poleks, annaksid veateate:

```{r eval = FALSE}
select(df$arvud) # antakse ette tunnus andmestikust ehk sisuliselt üks vektor
mean(df) # antakse ette kogu andmestik, funktsioon ei oska kogu andmestikuga midagi peale hakata
```
